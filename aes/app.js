(function () {
  ...
  function updateInputLabels() {
    ...
    if (fmt === "hex") {
      ...
      els.dataInput.placeholder = mode === "encrypt"
        ? "Contoh plaintext hex: 6bc1bee22e409f96e93d7e117393172a"
        : "Contoh ciphertext hex: 3ad77bb40d7a3660a89ecaf32466ef97";
    } else {
      ...
      els.dataInput.placeholder = mode === "encrypt"
        ? "Contoh plaintext teks: CipherHikmal AES"
        : "Dekripsi mode teks tetap memerlukan ciphertext yang berasal dari 16 byte block.";
    }
  }

  function matrixCellClass(kind, before, after, r, c) {
  ...
  function renderMatrix(state, options = {}) {
    ...
    html += `<table class="matrix-table" aria-label="${escapeHtml(title || "State Matrix")}"><thead><tr><th></th><th>c0</th><th>c1</th><th>c2</th><th>c3</th></tr></thead><tbody>`;
    for (let r = 0; r < 4; r++) {
      html += `<tr><th>r${r}</th>`;
      for (let c = 0; c < 4; c++) {
        html += `<td class="${matrixCellClass(kind, before, state, r, c)}">${A.hx(state[r][c])}</td>`;
      }
      html += `</tr>`;
    }
    html += `</tbody></table>`;
  ...
  function renderOperationStep(step, index) {
    const keyMatrix = step.key ? renderMatrix(step.key, { kind: "key", title: "Round Key" }) : "";
    const beforeMatrix = step.before ? renderMatrix(step.before, { kind: "base", title: "Before" }) : "";
    const afterMatrix = renderMatrix(step.after, {
      kind: step.kind,
      before: step.before,
  ...
  function renderKeyExpansion(exp) {
    ...
    const roundKeyCards = exp.roundKeys.map((rk, i) => `
      <details class="rounded-2xl border border-white/10 bg-white/[0.03] p-4" ${i === 0 ? "open" : ""}>
        ...
        <div class="mt-4">${renderMatrix(A.bytesToState(rk), { kind: "key", title: `Round Key ${i}` })}</div>
      </details>`).join("");
    ...
    return `
      <details id="key-expansion" class="glass rounded-[1.75rem]" open>
        <summary class="flex cursor-pointer items-center justify-between gap-4 p-5 sm:p-6">
          <div>
            <div class="text-xs font-black uppercase tracking-[0.18em] text-violet-300">Key Expansion</div>
            <h3 class="mt-1 text-xl font-black">Pembangkitan Kunci Ronde</h3>
            <p class="mt-1 text-sm leading-6 text-slate-400">Menampilkan W[0] sampai W[43], fungsi g, Rcon, dan RK0 sampai RK10.</p>
          </div>
          <span class="chevron rounded-full border border-white/10 px-3 py-2 text-slate-300">›</span>
        </summary>
        <div class="space-y-6 border-t border-white/10 p-5 sm:p-6">
          <div class="grid gap-4 lg:grid-cols-2">
            ${renderMatrix(initialKeyState, { kind: "key", title: "State Awal Kunci" })}
  ...
  function renderSummary(trace) {
    ...
    return `
      <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div class="details-tag">Output Akhir</div>
        ...
        <button id="copyBtn" class="btn-primary shrink-0">Copy to Clipboard</button>
      </div>
      <div class="mt-5 grid gap-3 text-xs text-slate-300 sm:grid-cols-3">
        ...
        <div class="info-card"><b class="text-slate-100">Round</b><br>10 ronde + initial round</div>
      </div>`;
  }

  function renderNav(trace) {
    const items = [
      { id: "key-expansion", label: "Key Expansion" },
      ...trace.sections.map(s => ({ id: s.id, label: s.title })),
      { id: "reference", label: "Tabel Referensi" }
    ];
  ...
  function renderAll(trace) {
    ...
    copyBtn.addEventListener("click", async () => {
      try {
        ...
        showMessage("success", "Output hex berhasil disalin ke clipboard.");
      } catch (_) {
        showMessage("info", "Clipboard API tidak tersedia. Silakan salin manual output hex yang ditampilkan.");
      }
    });
    ...
    els.visualization.innerHTML = `
      <section class="empty-state">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div class="max-w-3xl">
            <div class="details-tag">Siap Digunakan</div>
            <h2 class="mt-3 text-2xl font-black text-white">Masukkan data untuk memulai simulasi AES-128</h2>
            <p class="mt-3 details-note">
              Anda dapat mengisi input secara manual, menggunakan contoh input yang disediakan, atau memuat test vector NIST. Setelah proses dijalankan, CipherHikmal akan menampilkan Key Expansion, visualisasi ronde enkripsi atau dekripsi, dan output akhir dalam format heksadesimal.
            </p>
          </div>
          <div class="grid w-full max-w-md gap-3 text-sm text-slate-300">
            <div class="info-card">
              <div class="info-card-label">Contoh Hex</div>
              <div class="info-card-value font-mono text-sm">6bc1bee22e409f96e93d7e117393172a</div>
            </div>
            <div class="info-card">
              <div class="info-card-label">Contoh Kunci</div>
              <div class="info-card-value font-mono text-sm">2b7e151628aed2a6abf7158809cf4f3c</div>
            </div>
            <div class="info-card">
              <div class="info-card-label">Contoh Teks</div>
              <div class="info-card-value text-sm">CipherHikmal AES</div>
            </div>
          </div>
        </div>
      </section>`;
    els.roundNav.innerHTML = `<a class="nav-link active" href="#controls">Panel Input</a><a class="nav-link" href="#reference">Tabel Referensi</a>`;
  ...
  function run() {
    try {
      ...
      showMessage("success", `${expected} berhasil dihitung. Output: ${trace.outputHex}`);
    } catch (err) {
  ...
  function loadVector() {
    els.mode.value = "encrypt";
    els.inputFormat.value = "hex";
    els.dataInput.value = "6bc1bee22e409f96e93d7e117393172a";
    els.keyInput.value = "2b7e151628aed2a6abf7158809cf4f3c";
    updateInputLabels();
    run();
  }
  ...
  function fillExampleHex() {
    ...
    els.keyInput.value = "2b7e151628aed2a6abf7158809cf4f3c";
    updateInputLabels();
    showMessage("info", "Contoh input hex telah dimuat. Klik 'Jalankan Proses' untuk memulai.");
  }

  function fillExampleText() {
    els.mode.value = "encrypt";
    els.inputFormat.value = "text";
    els.dataInput.value = "CipherHikmal AES";
    els.keyInput.value = "2b7e151628aed2a6abf7158809cf4f3c";
    updateInputLabels();
    showMessage("info", "Contoh input teks telah dimuat. Klik 'Jalankan Proses' untuk memulai.");
  }
  ...
  function renderConstants() {
    els.constantTables.innerHTML = [
      renderConstTable("S-Box", A.SBOX),
      renderConstTable("Inverse S-Box", A.INV_SBOX),
      renderRcon(),
      `<div class="grid gap-4 lg:grid-cols-2">${renderSmallMatrix("MixColumns Matrix", A.MIX_MATRIX)}${renderSmallMatrix("InvMixColumns Matrix", A.INV_MIX_MATRIX)}</div>`
    ].join("");
  }
