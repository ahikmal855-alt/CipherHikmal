...
const SAMPLE_CASES = [
  { id: "main", name: "Contoh utama", plaintext: 0xD728, key: 0x4AF5, ciphertext: 0x24EC },
  { id: "second", name: "Contoh 2", plaintext: 0x1234, key: 0x5678, ciphertext: 0x0AEB },
  { id: "third", name: "Contoh 3", plaintext: 0x6F6B, key: 0xA73B, ciphertext: 0x0738 },
];
...
function normalizeHexInput(value) {
  return String(value ?? "").replace(/\s+/g, "").replace(/^0x/i, "").toUpperCase();
}

function validateSAESField(inputElement, errorElement, label) {
  const format = elements.inputFormat.value;
  const normalized = format === "hex" ? normalizeHexInput(inputElement.value) : normalizeBinary(inputElement.value);
  let message = "";

  if (!normalized) {
    message = `${label} wajib diisi.`;
  } else if (format === "hex" && !/^[0-9A-F]+$/.test(normalized)) {
    message = `${label} heksadesimal hanya boleh berisi 0-9 dan A-F.`;
  } else if (format === "hex" && normalized.length !== 4) {
    message = `${label} harus tepat 4 digit heksadesimal; saat ini ${normalized.length} digit.`;
  } else if (format === "bin" && !/^[01]+$/.test(normalized)) {
    message = `${label} biner hanya boleh berisi angka 0 dan 1.`;
  } else if (format === "bin" && normalized.length !== 16) {
  ...
  return { valid: !message, normalized, radix: format === "hex" ? 16 : 2 };
}
...
function updateModeUI() {
  ...
  const isHex = elements.inputFormat.value === "hex";
  elements.dataLabel.textContent = decrypt ? "Ciphertext 16-bit" : "Plaintext 16-bit";
  elements.dataHelp.textContent = isHex
    ? `Masukkan ${decrypt ? "ciphertext" : "plaintext"} tepat 4 digit heksadesimal.`
    : `Masukkan ${decrypt ? "ciphertext" : "plaintext"} tepat 16-bit. Spasi antarnibble diperbolehkan.`;
  elements.dataInput.placeholder = isHex ? "Contoh: D728" : "Contoh: 1101 0111 0010 1000";
  elements.keyInput.placeholder = isHex ? "Contoh: 4AF5" : "Contoh: 0100 1010 1111 0101";
  updateCounts();
...
function updateCounts() {
  const isHex = elements.inputFormat.value === "hex";
  const dataLength = isHex ? normalizeHexInput(elements.dataInput.value).length : normalizeBinary(elements.dataInput.value).length;
  const keyLength = isHex ? normalizeHexInput(elements.keyInput.value).length : normalizeBinary(elements.keyInput.value).length;
  elements.dataCount.textContent = isHex ? `${dataLength}/4 digit` : `${dataLength}/16 bit`;
  elements.keyCount.textContent = isHex ? `${keyLength}/4 digit` : `${keyLength}/16 bit`;
}

function getSelectedSample() {
  const selectedId = elements.exampleSelect.value;
  return SAMPLE_CASES.find((sample) => sample.id === selectedId) ?? SAMPLE_CASES[0];
}
...
function fillSelectedExample(runImmediately = false) {
  ...
  const isHex = elements.inputFormat.value === "hex";
  elements.dataInput.value = isHex ? hexWord(inputValue) : groupedBinary(inputValue);
  elements.keyInput.value = isHex ? hexWord(sample.key) : groupedBinary(sample.key);
  updateCounts();
  clearInputErrors();
...
elements.form.addEventListener("submit", (event) => {
  ...
  const dataValidation = validateSAESField(
    elements.dataInput,
    elements.dataError,
    mode === "encrypt" ? "Plaintext" : "Ciphertext",
  );
  const keyValidation = validateSAESField(elements.keyInput, elements.keyError, "Kunci");
  ...
  const data = parseInt(dataValidation.normalized, dataValidation.radix);
  const key = parseInt(keyValidation.normalized, keyValidation.radix);
...
function initialize() {
  ...
  // Test vector: P=D728, K=4AF5 → C=24EC; dekripsi harus kembali ke D728.
  const encrypted = encryptSAES(0xD728, 0x4AF5).output;
  const decrypted = decryptSAES(0x24EC, 0x4AF5).output;
  console.assert(encrypted === 0x24EC, `Self-test enkripsi gagal: ${hexWord(encrypted)}`);
  console.assert(decrypted === 0xD728, `Self-test dekripsi gagal: ${hexWord(decrypted)}`);
